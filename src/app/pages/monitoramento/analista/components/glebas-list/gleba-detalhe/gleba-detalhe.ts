...
export class AppGlebaDetalheComponent implements OnInit {
  ...
  private mapInstance?: any; // Alterado para 'any' para evitar dependência de tipo direta no SSR
  ...
  private async inicializarMapaGeometria(geojsonStr?: string): Promise<void> {
    // 1. Garante que só executará no Navegador (ignora o SSR do Node)
    if (!isPlatformBrowser(this.platformId) || !this.mapaElement || !geojsonStr) return;

    // 2. Importa a biblioteca Leaflet dinamicamente
